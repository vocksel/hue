return function()
	...
	it("should return true for light colors", function()
		for _, color in ipairs(lightColors) do
			expect(isLight(color)).to.equal(true)
		end
	end)

	it("should return false for dark colors", function()
		for _, color in ipairs(darkColors) do
			expect(isLight(color)).to.equal(false)
		end
	end)
end
